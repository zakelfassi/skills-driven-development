...
- 2026-07-01 — **Stop-gate follow-up fix #4** (claude-fable-5, flagged by the Codex stop-time review): `git status` emits cwd-relative paths, so when SessionStart and Stop ran from different directories (subdir sessions) the baseline hash keys didn't line up and a pre-dirty edit was missed/misattributed. `repoSnapshot` now anchors to `git rev-parse --show-toplevel` and runs status + hashes files from the repo root, so paths and hashes are cwd-independent. +1 regression test (SessionStart from root, edit+Stop from a subdir → blocks); 934 total green.
