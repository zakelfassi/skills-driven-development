...
- **Harness mirror** — a harness-expected skills directory (`.claude/skills`, `.codex/skills`, `.cursor/skills`, `.github/skills`, etc.) that points at the canonical `skills/` directory. On Unix it's a symlink; on Windows it's a file copy tracked in `.skdd-sync.json`. Managed by `skdd link` — **never edit directly**. The mirror exists so each harness can discover skills at its conventional path without N copies drifting in parallel.
